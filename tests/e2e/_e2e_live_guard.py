"""E2e live-instance guard (issue #191/#194).
...
def require_disposable_instance(
    ...
    *,
    ...
    Returns ``True`` when the caller should reclaim (kill + fresh restart)
    an already-occupied port because ``live_flag_env`` is set to ``"1"``, and
    ``False`` when it should simply boot fresh (the common case — the port is
    free, or nothing calls for reclaiming). Never returns while an occupied
    port has no opt-in: calls ``pytest.exit(..., returncode=2)`` instead,
    naming the flag, so a bare e2e run stops rather than silently killing or
    adopting a process it didn't start.
    ...
    if port_is_in_use(port, host) and not live_opt_in:
        pytest.exit(
            ...
            f"adopt it. Set {live_flag_env}=1 to explicitly allow "
            "reclaiming this port (kill + fresh restart), or free the port "
            "yourself first.",
            returncode=2,
        )
    if live_opt_in:
        print(f"[e2e] {live_flag_env}=1 - reclaiming {host}:{port}")
    else:
