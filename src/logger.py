...
* An in-memory ring buffer, which the Streamlit UI renders live via
  ``app/log_panel.py`` (``stream_to_streamlit``/``render_log_panel``).
  This module never imports ``streamlit`` — the UI-import boundary keeps
  it under ``app/`` (ruff ``TID251``).
...
# ---------------------------------------------------------------------------
# In-memory ring buffer (rendered in the UI by app/log_panel.py)
# ---------------------------------------------------------------------------
