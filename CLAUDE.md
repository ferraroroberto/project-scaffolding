...
- **Tokens come from the spec, not from you.** Wire your CSS custom properties (your app's `:root` / `[data-theme]` blocks) to `~/.claude/design.md` (light) + `~/.claude/design.dark.md` (dark) — colors, typography, spacing, radii. **Don't** copy the spec into your repo; **don't** invent a second accent or per-app palette. `/design-sync` reports drift.
...
**Each project declares a `## UX surface` block in its own `CLAUDE.md`** — the per-project *instance* the skills read, as `## CI expectations` does; don't inline these paths into the skill. **This scaffold ships the block below as a _live_ declaration, not a fenced sample**, so turning the gate on is a one-word edit (`design spec applies` → `yes`, then adapt the paths/views). Keep the block under this heading; `ux_surface.py` tolerates the descriptive `— …` suffix, so do **not** add a second `## UX surface` heading (the parser matches this one first and would read nothing).
...
- **Deterministic, diff-keyed — not a per-run LLM judgment.** Trigger is purely: does `git diff <main>...HEAD` intersect the declared `paths`? Yes → gate runs. No → skip silently and **state it** in the finish summary (`no UX surface touched`). Same path-keyed mechanism as the `## CI expectations` e2e-surface skip.
...
- **Gate at `/issue-finish` (and `/issue-yolo`), only when the diff touched the surface** — two legs:
  - **Token check, fix-now semantics.** Compare the touched UX files (CSS custom properties light + dark + the nav contract) to the spec and **fix material drift in this branch before merge** — unlike vanilla `/design-sync`, which files-and-defers a `design-drift` issue and stays as-is for the periodic sweep.
...
- **Ref:** the scaffold ships no starter FastAPI server, so this is documented, not seeded — a vendored `_vendored/settings/` component is a future step (`app/webapp/static/_vendored/README.md`). Reference implementations: `app-launcher` `app/webapp/routers/misc.py` + its `static/{index.html,main.js}`, and `home-automation`. (`#74`.)

## Webapp PWA static-asset cache-busting (`CachingStaticFiles` + fleet hash) *(PWA)*

iOS Safari — installed home-screen PWAs especially — heuristic-caches static assets served by a bare Starlette `StaticFiles` mount (only `ETag`/`Last-Modified`, **no explicit `Cache-Control`**): after a deploy + tray restart the device keeps running the **old cached JS/CSS** while `/api/version` reports the new build. Every fleet PWA ships the same fix: a **required convention**, not an optional extra.
...
- **Never add `DETACHED_PROCESS` on top — it neither suppresses the window nor escapes a subtree kill.** Measured from a console-less `pythonw` parent (`#221`): bare `CREATE_NO_WINDOW` suppressed the child's console every run, while `DETACHED_PROCESS` alone, `DETACHED_PROCESS | CREATE_NEW_PROCESS_GROUP`, and `CREATE_NO_WINDOW | DETACHED_PROCESS` **each** showed a visible window. `global-CLAUDE.md` owns the compatibility rule (mutually exclusive, `local-llm-hub#282`) — defer to it. Keeping a long-lived child alive across a tray teardown is a **re-parenting** problem, not a creation-flag one: `taskkill /T` walks the PID tree, which no creation flag changes, so use the `cmd /c start` re-parenting below.
...
- **Cap the fan-out — prose only.** Polling N devices/endpoints concurrently should bound concurrency (an `asyncio.Semaphore(N)` or equivalent around the poll loop) rather than firing all N every tick. Write it inline at the poll loop; too thin and framework-specific for a vendored wrapper.
...
- **The mechanism is shared; the rules are declared per-project.** `scripts/classify_e2e.py` reads an `[e2e]` table from the repo's own `.fleet.toml`, so a `paths → tier` map extends the declared-paths convention rather than adding a file. TOML so the classifier loads it with stdlib `tomllib` — zero custom parsing — and the rules stay versioned next to the code they classify. **`.fleet.toml` is the single auditable home for the routing table.**
...
**Retry inside the capture, never after it.** Capture-once must still survive a *transient* failure at capture time: a bounded retry (a few attempts, short backoff) inside the capture call absorbs it without breaking the "captured once, at process start" semantic. Re-resolving later — e.g. on each `/api/version` request — is the tempting fix and is wrong: it makes `git_sha` track live git state instead of what the process actually loaded, so a stale process can report a fresh `HEAD` as a confident **"fresh"** — strictly worse than `unknown`, and a direct violation of this section's own never-a-confident-false rule. Once the retries are exhausted, the identity stays `unknown` for that process's lifetime, permanently and honestly — not silently retried again outside the capture path. (Observed: `app-launcher`'s session-host reported `unknown` for 8 days off one transient `git` failure at import — `app-launcher#825`.)

**A detached/console-less child needs a real log destination, or its own failure explanation is discarded.** Same gap the tray watchdog breadcrumb (`#201`) closes for the tray itself — `pythonw` has no `sys.stderr`, so a child's default `logging.basicConfig()` handler writes to a stderr nobody reads — applies identically to any service child spawned detached (a session-host, a worker), not just the tray process. Piping that child's `stdout`/`stderr` to `DEVNULL` throws away exactly the diagnostics that would explain a frozen capture or a dead process. Give every detached child a real file target (append mode); fall back to `DEVNULL` only when the log path itself is unwritable, never as the default.
...
**Ref:** the convention + the per-project declaration shape live here; the skill-side enforcement (reading the declaration, checking the liveness field, wording the finish summary) lives in `fleet-config`'s `skills/issue-finish/SKILL.md`. Reference implementation: `app-launcher`'s `src/build_info.py` (shared git-sha + capture-timestamp helper, with `build_identity(attempts=3, backoff_seconds=0.3)` retrying inside the capture) + `/api/version`'s `session_host` sub-block + `scripts/restart-session-host.ps1` (`-Confirm`-gated, manual-only, never wired into a normal ship flow) + `verify-before-ship.ps1`'s advisory warning reusing `classify_e2e.py`'s routing output (`app-launcher#615`). `build_info` is not yet a catalogued `[components]` entry — small, stdlib-only, and now carrying non-obvious retry/logging semantics worth having in exactly one place; cataloguing it is tracked separately (`#247`). (`#199`; retry + detached-logging corollary `#246`; source instances `home-automation#314`, `app-launcher#611`/`#615`/`#825`/`#826`.)
