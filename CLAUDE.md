...
- **Default to isolation — boot a disposable instance; refuse before silently adopting.** With no opt-in env var set, the fixture boots its own fresh instance and, if the target port is already occupied, **refuses** (`pytest.exit`, naming the flag) rather than killing or reusing whatever's there — a bare `pytest tests/e2e` must never silently drive a live app the harness didn't start. Opt-in to reclaiming an occupied port is **one loudly-named env var per project** (`LAUNCHER_E2E_LIVE`, this scaffold's `STREAMLIT_E2E_LIVE`) — never an opt-**out** flag (`E2E_FORCE_AUTOBOOT=1`-shaped names get the polarity backwards: forgetting to set them is what silently re-enables adoption). Log which instance — disposable vs. reclaimed-live — the suite is driving and why, so a hung run is diagnosable from its own output. (Motivating incident: `project-scaffolding#191` — `home-automation`'s suite silently adopted the live tray with no route mocks, driving real MELCloud/RISCO/SMA/Tuya calls at ~15–20x baseline and, via ephemeral-TCP-port exhaustion, taking every fleet web app offline for minutes; audit found the same inverted opt-out polarity in `whatsapp-radar`, `voice-transcriber`, and `photo-ocr`.) Reference implementation: `app-launcher`'s `tests/e2e/conftest.py` (`LAUNCHER_E2E_LIVE` opt-in + `_require_live_tray` guard; `--e2e-autoboot` never adopts the live session-host on `:8446`, always spawns its own on a free port). This scaffold's own `tests/e2e/conftest.py` `streamlit_app` fixture demonstrates the minimal shape.
...
- **Not a `/propagate-vendored` component.** The guard's *shape* — check occupied → refuse-with-named-flag → conditional reclaim → spawn-fresh → log — is the generalizable convention above; the concrete boot/health-check mechanics (Streamlit subprocess + `/_stcore/health` here, `uvicorn` + `/healthz` for a FastAPI PWA, a session-host's own port for a PTY-backed app) differ per project shape and aren't byte-identical across repos, unlike `nav-tabs.js` or `single_instance.py`. Each adopter reimplements the shape against its own health signal and stateful-sidecar list rather than copying a vendored file; `/propagate-vendored` stays scoped to byte-identical UI components.
