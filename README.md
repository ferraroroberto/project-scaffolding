...
- `scripts/gen_tailscale_cert.py` — copy-to-adapt HTTPS cert generator (provisions a real Let's Encrypt leaf via `tailscale cert` for a tailnet MagicDNS name; `--check` auto-renews within ~30 days of expiry). **Not** in the `[components]` catalog below and not vendor-verbatim (#232) — every real adopter had already adapted it (its own `NO_WINDOW` import, `grocery-shopping-automation`'s different `CERT_DIR`), so byte-identity never held. Adapt on arrival, like `tray.bat.template`. See `docs/app-onboarding.md` §2.
...
- `app/webapp/static/_vendored/` — **vendor-verbatim web-app UI components** for the FastAPI + static PWA shape: the fleet design system's *implementation* layer (the *spec* lives in `fleet-config`'s `design.md`/`design.dark.md`). Components: `base/` — element-level rules linked before every other stylesheet, so bare form controls inherit the app's font and text color; `nav/` — the canonical floating bottom-tab navigation; `icons/` — the inline Lucide icon sprite + `icon()` helper; and the canon extracted from the home-automation polish round (design.md v2 "Component contracts"): `card/`, `disclosure/`, `modal/`, `empty-state/`, `switch/`, `icon-tile/`, `button/`, `range-tab/`, `page-foot/`, `home-head/`, `select-native/`, `action-row/` (the tap-the-row list row, fleet-config#965), and `text-size/` (the Small / Default / Large setting plus its pre-paint boot script, fleet-config#967). `demo.html` is the component gallery (open over HTTP, light + dark), driven by the `tests/e2e/test_vendored_components.py` render harness. Copy a component folder byte-for-byte; adapt only your markup + token values. See `app/webapp/static/_vendored/README.md` and `CLAUDE.md` ("Web-app visual identity").
...
- `tests/e2e/_e2e_live_guard.py` — canonical, **vendor-verbatim** e2e live-instance guard (`#191`/`#194`/`#197`, manifest key `e2e_live_guard`): refuses (`pytest.exit`, code 2) rather than silently killing or adopting a live app already listening on a fixture's target port, unless a loudly-named env var opts in to *acting on* it — what "acting" means (read-only assertions, or a reclaim via the adopting repo's own canonical restart recipe, never a by-hand kill) is caller-owned and repo-specific, never prescribed by this file. Copy it byte-for-byte into an app's `tests/e2e/`; the port and the opt-in flag name are call-site arguments to `require_disposable_instance(port, flag_env_name)`. Proven by `tests/test_e2e_live_guard.py`. See `CLAUDE.md` ("End-to-end UI testing" — regression suite).
- `tests/e2e/_browser_sweep.py` — canonical, **vendor-verbatim** post-run sweep for leaked Playwright browser helpers (#203, #236): after the session ends it classifies every WebKit helper process and tree-kills (`taskkill /F /T`) only those that are really running, genuinely orphaned (dead parent, PID-reuse-checked), *and* whose working directory sits under the checkout this run owns — the leak that otherwise blocks `git worktree remove` with an unexplained "busy". Everything else gets its own verdict rather than a kill. Liveness is a tri-state plus unknown, not a bool: an exit code alone does **not** mean exited, and `GetProcessTimes`'s exit time is the discriminator (#236) — a process with an exit code but no exit time is `wedged` inside termination, unkillable *and* still holding its cwd, so it is reported (loudly when it pins this checkout) instead of being swallowed as a harmless `zombie`. Wired as `pytest_sessionfinish` in `tests/e2e/conftest.py`; also runs standalone (`python tests/e2e/_browser_sweep.py <path> [--dry-run]`, exit 1 when the path is pinned). Copy it byte-for-byte into an app's `tests/e2e/`; the scope path is the only call-site argument. Proven by `tests/test_browser_sweep.py`. See `docs/playwright-ui-testing.md` ("Post-run sweep for leaked browser helpers").
- `tests/e2e/conftest.py`'s `_neutral_driver_cwd` + `playwright` fixture override — the *fix* the sweep above can only report on (#236). Everything Playwright spawns inherits a working directory, so a suite run from `<repo>-wt-<N>` roots the driver, the browsers and every helper in the worktree, and a helper wedged inside termination pins it permanently. Starting the driver from `%TEMP%` (`gettempdir()`, never `mkdtemp()`) makes a wedge harmless. Pinned by `tests/e2e/test_helper_cwd_isolation.py`; freeing a directory that is *already* pinned is in `docs/playwright-ui-testing.md` ("Freeing a pinned directory") — rebooting the host is not the remedy.
- `src/pooled_http.py` — canonical, **vendor-verbatim** pooled outbound HTTP client (`#192`, manifest key `pooled_http`): a keep-alive `requests.Session` with a sized `HTTPAdapter`, built once and never reconfigured per call, plus `pooled_request()` which retries once on a dropped connection. Copy it byte-for-byte into an app's `src/`; the pool size and target URL are call-site arguments to `build_session(pool_size)` / `pooled_request(method, url, ...)`. Proven by `tests/test_pooled_http.py`. Reference measured impact: `app-launcher#605` (145 → 0 `TIME_WAIT` sockets). See `CLAUDE.md` ("Outbound-connection discipline").
- `src/no_window.py` — the repo's single definition of the Windows `CREATE_NO_WINDOW` subprocess flag (#209). Import `NO_WINDOW` and pass it as `creationflags=` on every spawn of an external console executable, instead of re-deriving the `sys.platform == "win32"` ternary per call site. **Not** vendor-verbatim, and the one documented exception is deliberate: a file that *is* copied byte-identical into adopter repos (`tests/e2e/_browser_sweep.py`, `scripts/classify_e2e.py`) keeps its own local definition, because an import of this module would not resolve in a consumer's tree. See `CLAUDE.md` ("Windows console-subprocess suppression").
- `src/runtime_data.py` — canonical, **vendor-verbatim** runtime-data location resolver (`#243`, manifest key `runtime_data`): `runtime_db_path(app, filename, env_var=...)` returns `<root>/<app>/<filename>`, root taken from `FLEET_DATA_ROOT` or defaulting to `C:\sqlite` on Windows / `$XDG_DATA_HOME/sqlite` elsewhere, so an always-on service's SQLite writes land on a fast local drive instead of whichever drive the repo was cloned onto. The app's pre-existing full-path override (`TELEMETRY_DB_PATH`, `WR_DB_PATH`, …) stays highest precedence so test harnesses keep working. Stdlib-only, side-effect-free at import. Proven by `tests/test_runtime_data.py`. See `CLAUDE.md` ("Runtime data lives on a fast drive").
...
- `brand/` + `scripts/brand_gen.py` — the fleet icon/brand generator: `brand/catalog.json` catalogs 22 curated, vendor-verbatim Lucide SVG masters (one per project identity); `brand_gen.py`'s `render_set()` rasterizes a chosen master into every size the fleet needs — favicon, `apple-touch-icon`, Android maskable, Windows tray `.ico`, Stream Deck button. A downstream project calls it from a thin `scripts/gen_icons.py`. See `docs/brand-icons.md`.
...
- `.fleet.toml`'s `[components]` table + `scripts/vendored_catalog.py` — the canonical, machine-readable catalog of every vendor-verbatim component this scaffold publishes (`#230`): `<manifest key> = { src = "<path in this repo>" }`. **Only this repo carries it**; an adopter records what it copied in its own `[vendored]` table instead. It exists because `/propagate-vendored <component>` builds its adopter list from those `[vendored]` entries, so a repo carrying a component it never declared is invisible to the wave — the fan-out re-vendors the declared repos, reports success, and leaves the rest stale (`#228` reached one repo out of seven exactly that way). `fleet-config`'s `vendored_drift.py` reads this table to hash every fleet repo's copy of a *known* component path and name the undeclared carriers, and `scripts/verify-before-ship.ps1` derives its `mypy --strict` gate list from it rather than keeping a second hand-maintained copy. Add a component here in the same PR that adds it to the tree — `tests/test_vendored_catalog.py` fails otherwise.
...
app/
  ...
  webapp/static/_vendored/  vendor-verbatim web-app UI components (base/, nav/, icons/, card/, disclosure/, modal/, empty-state/, switch/, icon-tile/, button/, range-tab/, page-foot/, home-head/, select-native/, action-row/, text-size/ + demo.html gallery)
src/
  config.py                 paths + env-driven settings
  logger.py                 the elegant logger
  no_window.py              the one CREATE_NO_WINDOW flag every spawn imports
  pooled_http.py            vendor-verbatim pooled outbound HTTP client (keep-alive session + single-retry dispatch)
  runtime_data.py           vendor-verbatim SQLite-location resolver (C:\sqlite\<app>\, off the spinning disk)
...
brand/                      curated, vendor-verbatim Lucide SVG masters + catalog.json (fleet icon identity)
scripts/
  brand_gen.py               render_set() — generates favicon/apple-touch/maskable/tray/Stream-Deck icons from a brand master
...
- **Headless e2e** (`tests/e2e/`) — `pytest-playwright` against a real browser. The `streamlit_app` session fixture (`tests/e2e/conftest.py`) **boots a disposable instance and refuses an occupied port** (`#191`/`#194`): before booting anything it checks the e2e port through the vendor-verbatim guard (`_e2e_live_guard.py`'s `require_disposable_instance`) and, with no opt-in set, ends the run rather than killing or adopting a process it did not start — a bare `pytest tests/e2e` must never drive a live app. On a free port it boots fresh against the current code on disk via `tests/_streamlit_lifecycle.py`, so the suite can never pass against a stale process. Set `STREAMLIT_E2E_LIVE=1` to explicitly permit reclaiming an occupied port (see below); if a run exits with the guard's refusal, that flag — not a manual kill — is the documented way past it.
  - The suite is six files with **more test functions and collected items than the under-15-per-project target** (recount with `pytest tests/e2e --collect-only -q` rather than trusting a hardcoded number here — browser and viewport parametrisation makes the collected-item count drift independently of the function count): `test_smoke.py` (the example smoke test), `test_geometry_helper.py` (the rendered-geometry helper), `test_helper_cwd_isolation.py` (the neutral-driver-cwd fixture), `test_tray_lifecycle_behavior.py` (the tray lifecycle), and the two vendored-component harnesses `test_vendored_components.py` + `test_vendored_nav.py`. Expand per the regression-suite rules in `CLAUDE.md` ("End-to-end UI testing") — the target is under 15 tests *per project*; this scaffold carries more because it is also the proving ground for the vendor-verbatim primitives it ships.
