...
- `tests/e2e/_browser_sweep.py` — canonical, **vendor-verbatim** post-run sweep for leaked Playwright browser helpers (#203): after the session ends it classifies every WebKit helper process and tree-kills (`taskkill /F /T`) only those that are really running, genuinely orphaned (dead parent, PID-reuse-checked), *and* whose working directory sits under the checkout this run owns — the leak that otherwise blocks `git worktree remove` with an unexplained "busy". Everything else gets its own verdict rather than a kill: an already-exited-but-handle-held `zombie` (unkillable and harmless — never a gate failure), a live-parent session, a sibling checkout, or an unreadable cwd. Wired as `pytest_sessionfinish` in `tests/e2e/conftest.py`; also runs standalone (`python tests/e2e/_browser_sweep.py <path> [--dry-run]`). Copy it byte-for-byte into an app's `tests/e2e/`; the scope path is the only call-site argument. Proven by `tests/test_browser_sweep.py`. See `docs/playwright-ui-testing.md` ("Post-run sweep for leaked browser helpers").
