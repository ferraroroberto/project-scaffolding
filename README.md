...
- `tests/e2e/_e2e_live_guard.py` — canonical, **vendor-verbatim** e2e live-instance guard (`#191`/`#194`, manifest key `e2e_live_guard`): refuses (`pytest.exit`, code 2) rather than silently killing or adopting a live app already listening on a fixture's target port, unless a loudly-named env var opts in to reclaiming it. Copy it byte-for-byte into an app's `tests/e2e/`; the port and the opt-in flag name are call-site arguments to `require_disposable_instance(port, flag_env_name)`. Proven by `tests/test_e2e_live_guard.py`. See `CLAUDE.md` ("End-to-end UI testing" — regression suite).
