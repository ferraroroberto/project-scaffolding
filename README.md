...
- `app/webapp/static/_vendored/` — **vendor-verbatim web-app UI components** for the FastAPI + static PWA shape: the fleet design system's *implementation* layer (the *spec* lives in `fleet-config`'s `design.md`/`design.dark.md`). Components: `nav/` — the canonical floating bottom-tab navigation (`nav-tabs.js` + `nav-tabs.css` + `nav-tabs.html`); `icons/` — the inline Lucide icon sprite + `icon()` helper (`icons-sprite.html` + `icons.js`). Copy a component folder byte-for-byte; adapt only your markup + token values. See `app/webapp/static/_vendored/README.md` and `CLAUDE.md` ("Web-app visual identity").
...
- `docs/app-onboarding.md` — didactic end-to-end playbook for standing up a new self-hosted PWA app from this scaffold: bootstrap the app, issue + trust the self-signed-CA HTTPS cert (`certutil -user -addstore Root ca.pem` + the "fully restart Chrome to pick up a new root" gotcha + leaf-cert expiry note), and the iPhone/Android PWA install (the `/install-ca` mobileconfig detour). Links to `docs/windows-tray.md` for the tray half. Read this when bringing up a new tray-resident webapp.
...
app/
  ...
  tray/tray_lifecycle.ps1   vendor-verbatim tray lifecycle helper
  tray/single_instance.py   vendor-verbatim named-mutex primitive for tray apps
  webapp/static/_vendored/  vendor-verbatim web-app UI components (nav/ = bottom-tab nav, icons/ = Lucide sprite)
src/
