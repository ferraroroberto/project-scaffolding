...
- `tray.bat` shells out to **Windows PowerShell 5.1** (`C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe`), running the whole lifecycle via one `-File %USERPROFILE%/.claude/tray/tray_lifecycle.ps1 launch ...` call against the ONE shared, machine-local copy owned by `fleet-config` (project-scaffolding#153). Keep that `.ps1` **ASCII-only** — a stray em-dash or other non-ASCII char breaks PS 5.1 parsing. (Write the prose with em-dashes; keep the *code* plain.)
...
- `tray.bat.template` itself must **also** stay ASCII-only, for a distinct reason: it opens with `chcp 65001` to switch the console to UTF-8, but `cmd.exe` read-ahead-buffers a chunk of the batch file for parsing at whatever codepage was active when the buffer was filled. A multi-byte UTF-8 character anywhere inside that pre-fetch window gets misparsed once the codepage switches mid-file, throwing spurious `'X' is not recognized as an internal or external command` errors on unrelated later lines — reproduced with a minimal `chcp 65001` + em-dash-in-a-REM-comment `.bat` invoked non-interactively (`cmd.exe //c` from Git Bash), confirmed fixed by removing the non-ASCII character (project-scaffolding#183). Same rule as the `.ps1` above, different underlying mechanism (cmd.exe batch read-ahead, not PS 5.1's parser).
...
Shelling once to the helper removes cmd output parsing from the lifecycle entirely (app-specific values — venv path, tray-match regex, owned ports, tray launch, optional version URL — are passed as plain arguments), so detection, reclaim, start, and verification behave **identically** whether `tray.bat` is run from an interactive console, the Startup folder, or a non-interactive agent shell. As of `project-scaffolding#153`, `tray_lifecycle.ps1` is **no longer vendored per-app** — it is the ONE shared, machine-local copy owned by `fleet-config`, exposed at `%USERPROFILE%\.claude\tray\` by that repo's `install.ps1` junction, so a fix made once is live for every tray the moment it merges. `single_instance.py` is unaffected and stays vendored (see "Single source of truth" below). `tray.bat` hard-errors if the shared helper is missing rather than silently no-op'ing back into the stale-adopt failure — the error names `fleet-config`'s `install.ps1` as the fix.
