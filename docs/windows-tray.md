...
**Verification is by served `git_sha` vs repo `HEAD`, never a `healthz` 200** — a stale adopted process passes a health check fine. The agent owns the bounded version-endpoint poll (see "The agent-side contract" below); `claude-config`'s `/issue-finish` invokes `tray.bat --restart` through a real Windows shell and **stops + surfaces** a `git_sha ≠ HEAD` mismatch rather than improvising process kills (`claude-config#89`).
