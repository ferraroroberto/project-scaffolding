...
**Tray + long-lived service.** A self-hosted PWA app runs its webapp under a Windows tray that owns the service port. Do **not** re-derive the tray lifecycle here — copy `tray.bat.template` → `tray.bat` (replace the four `__PLACEHOLDER__` tokens) and vendor **both** tray primitives verbatim: `app/tray/single_instance.py` (in-process single instance + race-safe adopt-or-spawn) and `app/tray/watchdog.py` (retry-with-backoff on the initial webapp spawn, the dead-vs-wedged health watchdog, and the `webapp/watchdog.log` breadcrumb trail — the only durable diagnostic under `pythonw`, which has no `sys.stderr`; see gotcha #5). `tray_lifecycle.ps1` (the detect → kill → reclaim → start → verify lifecycle) is **not** vendored (project-scaffolding#153): the template calls the ONE shared, machine-local copy owned by `fleet-config` at `%USERPROFILE%/.claude/tray/tray_lifecycle.ps1`, so the new app just needs `fleet-config` installed on the machine (`install.ps1` run) — nothing to copy. The full reasoning — idempotent single-instance start, the orphan-proof `tray.bat --restart` that reclaims owned ports by PID, and the agent-side bounded-poll restart contract — lives in [`windows-tray.md`](windows-tray.md). Read that before writing or fixing the restart; this playbook assumes the tray is already wired per that doc.
...
- `scripts/gen_tailscale_cert.py` (this scaffold) — the preferred-path generator: provision a real Let's Encrypt leaf via `tailscale cert` plus the `--check` auto-renew leg. **Copy-to-adapt, not vendor-verbatim** (#232) — every real adopter's copy differs (its own `NO_WINDOW` import, `grocery-shopping-automation`'s different `CERT_DIR`), so it is not in the scaffold's `[components]` catalog. Reference wire-up into a webapp launcher: `grocery-shopping-automation`'s `webapp.bat`.
